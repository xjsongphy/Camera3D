...
三个向量 $\mathbf{x}$、$\mathbf{t}$ 和 $\mathbf{x} - \mathbf{t}$（后者等于 $\mathbf{R}^T \mathbf{x}'$，即 $\mathbf{x}'$ 在第一台相机坐标系下的表示）共面，这一**共面约束（coplanarity constraint）** 可用标量三重积表示为：
...
这一方程描述了两幅图像中对应点之间的代数约束。$\mathbf{E}$ 是一个 $3 \times 3$ 的矩阵，具有两个重要性质：其**秩为 2**（因为反对称矩阵 $[\mathbf{t}]_\times$ 的秩为 2），且**奇异值为 $[\sigma, \sigma, 0]$**（两个非零奇异值相等）。
...
需要注意的是，奇异值条件 $[\sigma, \sigma, 0]$ 是本质矩阵的**必要条件**而非充分条件。在 5-point 算法等求解过程中，通常需要将候选解通过 SVD 投影到本质矩阵流形上（强制奇异值为 $[\sigma, \sigma, 0]$），以确保解满足本质矩阵的全部约束。

### 2.3 八点算法

估计本质矩阵（或基础矩阵）最经典的线性方法是**八点算法（8-point algorithm）**（Longuet-Higgins, 1981）。将 $\mathbf{x}'^T \mathbf{E} \mathbf{x} = 0$ 展开，每一对匹配点给出一个线性方程。本质矩阵 $\mathbf{E}$ 有 9 个元素，但由于齐次等价性（$\mathbf{E}$ 定义在相差一个尺度因子的意义上），实际有 8 个自由度，因此至少需要 8 对匹配点。

将 $\mathbf{E}$ 的元素拉成向量 $\mathbf{e}$，每对匹配点贡献一个方程 $a_i^T \mathbf{e} = 0$，8 对点组成 $\mathbf{A} \mathbf{e} = \mathbf{0}$。解为 $\mathbf{A}$ 的最小奇异值对应的右奇异向量（即 SVD 的最后一列）。得到 $\mathbf{E}$ 后，再通过 SVD 将其投影到本质矩阵流形：$\mathbf{E} = \mathbf{U} \operatorname{diag}(\sigma, \sigma, 0) \mathbf{V}^T$。

八点算法简单直观，但对数据归一化敏感。**归一化八点算法**（Hartley, 1997）在求解前先将图像坐标平移并缩放到 $[-1, 1]$ 范围内，显著提升了数值稳定性，使其在实践中也具有良好的效果。
