...
题目三仍然沿用 `ffmpeg + COLMAP` 的基本流程，但重点从“直接重建”转向“如何在特征提取前屏蔽动态区域”。实现上，我将所有改进方法都统一为给 COLMAP 提供 image mask，从而避免行人、车辆等动态物体上的特征点进入后续匹配与建图流程。实验中保留 `raw` 作为基线，同时比较三种 mask 来源：人工设计的 `default mask`、基于时序差分的 `motion mask`，以及基于实例分割模型的 `yolo mask`。

### Mask 方案

`default mask` 是人工选取的先验区域。观察两段视频后，可以发现它们都有一个相对稳定、且动态目标较少出现的“安全区域”。因此我直接为每段视频手工指定一个固定相机遮罩：对 `S2-1` 保留图像顶部 `20%` 的区域，对 `S2-2` 保留左上 `1/4` 的区域。这个方法的优点是实现最简单、运行代价最低，而且能够直接验证一个问题：**只要人为排除大部分动态区域，SfM 是否会比 raw 更稳定**。它的局限也很明显，即遮罩完全依赖人工观察，泛化能力很弱，一旦相机视角或场景布局变化，就需要重新设计。

`motion mask` 的思路是利用相邻帧之间的亮度变化来检测动态区域。具体做法是：对当前帧分别与前一帧、后一帧做灰度差分，并先用一个小范围平移搜索粗略补偿相机运动；补偿后，若某些像素在前后对比中都表现出明显变化，则更可能属于独立运动的目标。将这些区域二值化后，再通过膨胀操作适当扩大遮罩范围，避免动态物体边缘仍残留特征点。这个方法的优点是不需要任何学习模型，完全基于视频本身即可得到 mask；但它对光照变化、相机运动补偿误差以及动态目标速度都较敏感，因此 mask 的稳定性通常不如语义分割方法。

`yolo mask` 的思路是直接用实例分割模型检测常见动态类别，并把这些实例区域合并成 COLMAP 可用的二值 mask。这里使用 YOLO 分割模型逐帧推理，并只保留 `person`、`bicycle`、`car`、`motorcycle`、`bus`、`truck` 等动态类别的分割结果。对于每一帧，将这些实例 mask 求并集，再保存为单通道图像，供 COLMAP 在特征提取阶段屏蔽对应区域。这个方法的优点是语义上更直接，能够显式针对“人和车”这类动态物体进行抑制，通常也比简单差分更稳定；缺点是需要额外的模型依赖和推理时间，而且当检测漏掉动态目标或分割边界不准确时，仍可能保留干扰特征。
