...
| 方法 | 注册率 | 可靠点比例 | 重投影误差中位数 | 重投影误差 P90 | 轨迹长度中位数 | 跳变比例 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
题目四尝试了两类不依赖 ground truth 的指标：一类只使用给定相机位姿，检查轨迹本身的局部平滑性；另一类从原视频重新提取匹配点，用给定位姿三角化后统计对极/重投影误差。最终没有把多个单项硬混成一个分数，而是分别分析每个指标的区分能力。

### 单项指标

- **局部锯齿程度** `zigzag_score`：对每个相机中心取前后 3 个相邻点，拟合局部 3D 主方向直线；统计当前点到该局部直线的垂直残差，并用全轨迹 median step 归一化。最终分数由残差超过 `2 × median step` 的比例和残差 95 分位数组成，越低越好。
- **相邻帧加速度跳变** `smooth_jump_ratio`：按真实帧间隔计算速度和加速度，统计加速度超过 `4 × median(|a|)` 的比例，越低越好。
- **旋转加速度跳变** `rot_accel_jump_ratio`：类似平滑指标，但针对旋转角速度计算加速度，统计旋转加速度超过 `4 × median(|α|)` 的比例，越低越好。
- **对极距离违例比例** `epi_dist_px`：从原视频按 `fps=16` 抽取与位姿对应的帧，对相邻采样帧做 SIFT 双向匹配，使用给定内参和位姿计算对称对极距离，统计超过阈值（`1.5px`）的比例，越低越好。
- **重投影误差违例比例** `reproj_err_px`：使用给定内参和位姿对匹配点进行三角化，统计重投影误差超过阈值（`2.0px`）的比例，越低越好。

![Task4 individual metrics](report_assets/task4/task4_individual_metrics.png)

几个指标没能有效区分两种 case，仅 `zigzag_score` 和 `smooth_jump_ratio` 有效区分了 04 和 05 两个 bad case 。

> 我大胆猜测更好的指标需要重新计算点云，但是由于开始对题目的理解是不运行 COLMAP Mapper 的，仅从位姿数据进行判别，因此没有足够的时间打磨点云辅助的指标。