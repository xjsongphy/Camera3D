...
from lab3.common import require_tool, run_cmd, timed_block
    ...
    def run(self, context: ReconstructionContext) -> None:
        ...
        if not self.config.skip_process_data:
            ...
            with timed_block("nerf_process_data", context.timings):
                run_cmd(cmd, dry_run=context.dry_run)
        ...
        with timed_block("nerf_train", context.timings):
            run_cmd(train_cmd, dry_run=context.dry_run)
