...
`Camera3D` 是一个基于 `uv` 管理的课程实验仓库，包含 Lab1（视频3D重建）和 Lab2（结构光优化）两个独立实验。

## 实验模块

- **[Lab 1: 基于视频的3D重建](docs/lab1/README.md)** — 静态与动态场景SfM、子序列分析、位姿质量评估
- **[Lab 2: 结构光Pattern自动优化](docs/lab2/README.md)** — Optical SGD优化、ZNCC Decoder、材质鲁棒性

---

## Lab 1: 基于视频的3D重建

`Camera3D` 的 Lab1 实现了基于单目视频的静态与动态场景3D重建，包含四个核心任务：
